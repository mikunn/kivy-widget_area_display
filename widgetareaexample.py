...
<MyBoxLayout>:
    ...
    GreenLabel:
        text: 'GreenLabel'
        canvas: self.show_widget_area()
        
    BlueLabel:
        text: 'BlueLabel'
        canvas: self.show_widget_area(color='b')
    
    GridLayout:
        rows: 1
        canvas: self.show_widget_area(color='orange', alpha=1, group='before')
        
        CustomColorLabel:
            size_hint: None, None
            size: 200, 40
            text: 'CustomColorLabel'
            canvas: self.show_widget_area(color=(.2, .7, .8, 1))
...
class CustomColorButton(Button):
    def __init__(self, **kwargs):
        ...
        self.show_widget_area(color=(0,1,1), alpha=0.8)
        
class AnotherCustomColorButton(Button):
    def __init__(self, **kwargs):
        super(AnotherCustomColorButton, self).__init__()
        self.show_widget_area(color=(0.7, 0.7, 0.3), alpha=0.8, group='after')
